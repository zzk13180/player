...
import { NewImageElement } from '@player/common/interfaces/final-object.interface'
...
export class FolderViewPipe implements PipeTransform {
  ...
  determineFolderProperties(files: ImageElement[]): FolderProperties {
    ...
    const starString: StarRating = <StarRating>(
      (<unknown>(Math.round(starAverage / totalStars - 0.5) + 0.5 || 0.5).toString())
    )
