...
export class SliderFilterComponent implements OnInit, OnDestroy {
  ...
  convertToIndex(value: number): number {
    const cutoff =
      (value / this.width) * (this.maximumValue - this.minimumValue) + this.minimumValue

    if (
      (this.lengthFilter || this.sizeFilter || this.timesPlayed || this.yearFilter) &&
      cutoff > this.maximumValue
    ) {
      return Infinity
    } else {
      return cutoff
