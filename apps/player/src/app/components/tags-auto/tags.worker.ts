...
function received(message: any): void {
  if (message.data.task === 1) {
    // console.log('task1');
    postMessage(
      getPotentialTwoWordTags(message.data.onlyFileNames, message.data.oneWordFreqMap),
    )
  } else if (message.data.task === 2) {
    // console.log('task2');
    postMessage(
      getCleanTwoWordMap(message.data.potentialTwoWordMap, message.data.onlyFileNames),
    )
  }
}
