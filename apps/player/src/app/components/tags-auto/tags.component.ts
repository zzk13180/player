...
import { slowFadeIn, donutAppear, metaAppear } from '../../animation/animations'
...
@Component({
  ...
  styleUrls: ['../search-input.scss', '../wizard-button.scss', 'tags.component.scss'],
...
export class TagsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.tagsService
      .generateAllTags(this.imageElemetService.imageElements, this.hubName)
      .then(() => {
        setTimeout(() => {
          this.showEdit = true
        }, 300)
        ...
        setTimeout(() => {
          if (this.filterInput) {
            // in case user already closed the modal
