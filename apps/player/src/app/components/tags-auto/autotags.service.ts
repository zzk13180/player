...
export const autoFileTagsRegex = /[\p{L}\d]+/gu
...
export class AutoTagsService {
  ...
  constructor(public autoTagsSaveService: AutoTagsSaveService) {}
  ...
  public generateAllTags(finalArray: ImageElement[], hubName: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      if (this.cachedHub !== hubName) {
        ...
        this.getPotentialTwoWordMap(this.onlyFileNames, this.oneWordFreqMap).then(
          potentialTwoWordMap => {
  ...
  private storeFinalArrayInMemory(finalArray: ImageElement[]): void {
    finalArray.forEach(element => {
      const cleanedFileName: string = (
        element.cleanName.toLowerCase().match(autoFileTagsRegex) || []
      ).join(' ')
  ...
  public canWeAdd(tag: string): boolean {
    ...
    } else if (this.oneWordFreqMap.has(tag)) {
      return false
    } else {
  ...
  private getPotentialTwoWordMap(
    ...
    return new Promise((resolve, reject) => {
      ...
      const worker = new Worker(new URL('./tags.worker', import.meta.url), {
        type: 'module',
      })
