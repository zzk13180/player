...
import { SettingsButtons } from '@player/common/todo/settings-buttons'
...
export class MetaComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.responseSubscription = this.renameResponse.subscribe(
      (data: RenameFileResponse) => {
        if (data) {
          console.log('Rename response:')
          console.log(data)

          if (this.video.index === data.index) {
            // make sure the message is about current component's video
            if (data.success) {
  ...
  tryRenamingFile() {
    ...
    const newFileName = `${this.renamingWIP}.${this.filePathService.getFileNameExtension(
      this.video.fileName,
    )}`
