...
import { metaAppear, breadcrumbWordAppear } from '../../animation/animations'
...
export class StatisticsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.eventSubscriptionMap.set(
      'serverDetails',
      this.serverDetails.subscribe((serverDetails: ServerDetails) => {
    ...
    this.eventSubscriptionMap.set(
      'inputFolder',
      this.inputFolderChosen.subscribe((folderPath: string) => {
        if (folderPath) {
          // first emit from subscription is `undefined`
    ...
    this.eventSubscriptionMap.set(
      'folderReconnect',
      this.oldFolderReconnected.subscribe(data => {
        if (data) {
          // first emit from subscription is `undefined`
    ...
    this.eventSubscriptionMap.set(
      'numberOfScreenshotsDeleted',
      this.numberScreenshotsDeleted.subscribe((deleted: number) => {
  ...
  handleInputFolderChosen(filePath: string) {
    ...
    if (!pathAlreadyExists) {
      ...
      console.log('⭐ start-watching-folder', nextIndex, filePath, false)
    }
  ...
  pickNextIndex(inputSource: InputSources) {
    ...
    const indexesAsNumbers: number[] = indexesAsStrings.map((item: string) =>
      parseInt(item, 10),
    )
  ...
  cleanScreenshotFolder(): void {
    ...
    console.log('⭐ clean-old-thumbnails', this.imageElementService.imageElements)
  }
  ...
  tellNodeStopWatching(itemSourceKey: number) {
    console.log('⭐ stop-watching-folder', itemSourceKey)
  }
  ...
  tellNodeStartWatching(itemSourceKey: number, path: string, persistent: boolean) {
    console.log('⭐ start-watching-folder', itemSourceKey, path, persistent)
  }
  ...
  animateThis(event) {
    ...
    setTimeout(() => {
      if (event.srcElement.classList) {
        // this might not even be needed it seems
